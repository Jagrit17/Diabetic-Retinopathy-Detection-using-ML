...
PATHS = {
'train_csv': r'C:\Programming\DR Detection using ML\Messidor2\messidor_data.csv',
'test_csv': r'C:\Programming\DR Detection using ML\Messidor2\messidor_data.csv',
'train_img': r'C:\Programming\DR Detection using ML\Messidor2\Messidor2img',
'test_img': r'C:\Programming\DR Detection using ML\Messidor2\Messidor2img'
}

# Column name standardization for diabetic retinopathy binary classification
COLUMN_MAPPING = {
    'image': ['id_code', 'image', 'img', 'image_name', 'image_id', 'Image'],  # Adjusted for 'id_code' in dataset
    'target': ['risk_of_macular_edema', 'macular_edema_risk', 'retinal_risk', 'diagnosis', 'adjudicated_dme', 'adjudicated_gradable', 'Risk of macular edema']  # Adjusted for your columns
}
...
try:
    ...
    print(f"Train set shape after cleanup: {train_df.shape}")
    print(f"Test set shape after cleanup: {test_df.shape}")
...
def preprocess_image(img_path, display=False):
    """Process images with error handling and display intermediate stages if needed"""
    try:
...
def build_dataset(df, img_dir, display_samples=False):
    """Create normalized dataset"""
    X, y = [], []
    for i, row in df.iterrows():
        img_path = os.path.join(img_dir, row['image'])
        features = preprocess_image(img_path, display=(display_samples and i < 1))  # Display only the first sample
...
# Build datasets
try:
    X_train, y_train = build_dataset(train_df, PATHS['train_img'], display_samples=True)
...
print("\nExecution completed successfully using Messidor II!")
